...
$(document).ready(function(){  //declare the page document for a function
    $("button#answer").click(function(){ 	//get the id from the select option, if changed option, do the ajax
        ...
        $.post("back/get_test_answer.php", {answer: ans, question: ques}, function(data) //use for posting data without using any form and without changing the page
        {
            if(data.status == true)
            {  
...
{
    if(!isset($_GET['test_id']) && !isset($_GET['question_id']))
    {
    ...
    <?php
        ...
        $sql1 = "SELECT COUNT(*) AS total FROM question INNER JOIN test ON question.test_id = test.test_id WHERE question.test_id = '$test_id'";
        $result1 = mysqli_query($conn,$sql1);
        while($rows = mysqli_fetch_array($result1))
        {
            $total = $rows['total'];
        }
        $sql2 = "SELECT * FROM question INNER JOIN test ON question.test_id = test.test_id WHERE question.test_id = '$test_id' AND question.question_id = '$question_id' ORDER BY question.question_id";
       ...
       <center><span id='result'></span></center> 
                ...
                <div class='card-footer'>
                    ...
                    <center>
                        <?php
                            $i = 1;
                            $sql4 = "SELECT * FROM question INNER JOIN test ON question.test_id = test.test_id WHERE question.test_id = '$test_id' ORDER BY question.question_id";
                            ...
                            while($rows = mysqli_fetch_array($result4))
                            {
                                ...
                                if($question_id == $id)
                                {
                                    echo "<button class='btn btn-outline-info active' onclick=\"location.href='student_answer_test.php?post_id=".$post_id."&test_id=".$test_id."&question_id=".$id."'\">$i</button>";
                                }
                                else
                                {
                                    echo "<button class='btn btn-outline-info' onclick=\"location.href='student_answer_test.php?post_id=".$post_id."&test_id=".$test_id."&question_id=".$id."'\">$i</button>";
                                }
                                $i++;
                            }
                        ?>

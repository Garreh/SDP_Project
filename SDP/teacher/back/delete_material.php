...
if(isset($_GET['post_id']) && isset($_GET['material_id']))
{
    ...
    {
        $sql1 = "DELETE material.* FROM material INNER JOIN post ON material.post_id = post.post_id WHERE material.post_id = '$post_id' AND material.material_id = '$material_id'";
        mysqli_query($conn,$sql1);

        echo "<script>alert('Material is Deleted!')</script>";
        echo "<script>window.location.href='../post_detail.php?group_id=".$group_id."&post_id=".$post_id."'</script>";
    }
    else
    {
        $sql1 = "DELETE material.* FROM material INNER JOIN post ON material.post_id = post.post_id WHERE material.post_id = '$post_id' AND material.material_id = '$material_id'";
        ...
        $sql2 = "DELETE section.* FROM section INNER JOIN material ON section.material_id = material.material_id INNER JOIN post ON material.post_id = post.post_id WHERE material.post_id = '$post_id' AND section.material_id = '$material_id'";
